...
mistral_data <- read_csv("mistral_output_separate_text_emoji.csv")[,-1]

identical(mistral_data$Author_ID, complete_data$Author_ID) # it's true, so we just add the sentiment score column to the complete data

complete_data$original_score <- mistral_data$sentiment_score
...
plot_interaction(input_data = sentiment_avg_neg, response_col = "score", 
                 file_name = "mistral_separate_neg_avg_txt_emoji", 
                 y_lower = 0, y_upper = 5, line_title_position = 4.5, 
                 ylab = "mistral_separate_neg_avg_txt_emoji")
...
plot_interaction(input_data = sentiment_avg_neg, response_col = "score_new", 
                 file_name = "mistral_separate_neg_avg_txt_emoji_weighted", 
                 y_lower = 0, y_upper = 5, line_title_position = 4.5, 
                 ylab = "mistral_separate_neg_avg_txt_emoji_weighted")
...
write.csv(wide, "mistral_separate_neg_avg_txt_emoji.csv", row.names = F)
write.csv(wide_weighted, "mistral_separate_neg_avg_txt_emoji_weighted.csv", row.names = F)