...
mistral_data <- read_csv("mistral_output_combine_text.csv")[,-1]  # we can't set mistral_data to complete since the make_wide_t function will extract the time-invariant variable from the complete data and the sentiment score from sentiment_score data which is from mistral_data; If the original mistral_data has the time_invariant variable then we are good to go, otherwise, no
...
plot_interaction(input_data = sentiment_score, response_col = "original_score", 
                 file_name = "mistral_combine_txt", 
                 y_lower = 0, y_upper = 10, line_title_position = 9.5, 
                 ylab = "mistral_combine_txt")
...
write.csv(wide, "mistral_combine_txt.csv", row.names = F)
