...
llama_data <- read_csv("llama_output_combine_text_emoji.csv")[,-1]  # we can't set llama_data to complete since the make_wide_t function will extract the time-invariant variable from the complete data and the sentiment score from sentiment_score data which is from llama_data; If the original llama_data has the time_invariant variable then we are good to go, otherwise, no
...
plot_interaction(input_data = sentiment_score, response_col = "original_score", 
                 file_name = "llama_combine_txt_emoji", 
                 y_lower = 0, y_upper = 10, line_title_position = 9.5, 
                 ylab = "llama_combine_txt_emoji")
...
write.csv(wide, "llama_combine_txt_emoji.csv", row.names = F)
