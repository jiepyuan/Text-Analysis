...
dat <- read.csv("afinn_separate_all_avg_txt_emoji.csv")
dat$id <- 1:nrow(dat) 
...
model {
  ...
  gamma00 ~ dnorm(0.2345, 0.1)
  gamma10 ~ dnorm(0.007056, 0.1)
...
alpha_init_1 <- matrix(rep(c(0.2345, 0.007056), max(long_dat$id_idx)), 
                       ncol = 2, byrow = TRUE)

alpha_init_2 <- matrix(rep(c(0.15, 0.02), max(long_dat$id_idx)), 
                       ncol = 2, byrow = TRUE)

alpha_init_3 <- matrix(rep(c(0.35, -0.01), max(long_dat$id_idx)), 
                       ncol = 2, byrow = TRUE)
...
inits_growth_1 <- list(
  ...
  gamma10       = 0.007056,
  sigma_epsilon = 3.333,
  Tau_u         = Omega_init, # Use the precision matrix computed from the LME results
  beta_i        = alpha_init_1
)
...
jags_mod_growth <- jags.model(
  ...
  inits    = list(inits_growth_1, inits_growth_2, inits_growth_3), 
  n.chains = 3,            
  n.adapt  = 5000          
)
...
trace_cols <- c("#0072B2", "#E69F00", "#000000")

for (p in trace_params_growth) {
  safe_p <- gsub("\\[|\\]", "_", p)
  png(file.path(out_dir, paste0("traceplot_", safe_p, ".png")),
      width = 800, height = 600)
  traceplot(samples_diag_growth[, p],
            main = paste("Traceplot of", p),
            col = trace_cols)
  dev.off()
}
...
summary_params_growth <- varnames(samples_growth)
summary_params_growth <- summary_params_growth[!grepl("^log_lik\\[", summary_params_growth)]
samples_summary_growth <- samples_growth[, summary_params_growth]
summary_results_growth <- summary(samples_summary_growth)

save(summary_results_growth,
     file = file.path(out_dir, "summary_results_GCM.RData"))

final_table_growth <- cbind(
  Mean     = summary_results_growth$statistics[, "Mean"],
  SD       = summary_results_growth$statistics[, "SD"],
  Lower_95 = summary_results_growth$quantiles[, "2.5%"],
  Upper_95 = summary_results_growth$quantiles[, "97.5%"]
)

print(final_table_growth)

write.csv(final_table_growth,
          file = file.path(out_dir, "GCM_model_parameter_estimates.csv"),
          row.names = TRUE)

rmse_growth <- summary_results_growth$statistics["rmse", "Mean"]
