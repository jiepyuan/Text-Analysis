...
model {
  ...
  # 1. Population mean prior for alpha_i: centered at the lme estimate 0.297
  mu_alpha ~ dnorm(0.297, 0.1) 
  
  # 2. Autoregressive coefficient prior: centered at the lme estimate -0.0386
  phi ~ dnorm(-0.0386, 0.1) 
...
jags_mod_ar1 <- jags.model(
  ...
  data     = jags_data_ar1,
  inits    = list(inits_ar1_1, inits_ar1_2, inits_ar1_3), 
  n.chains = 3,            
  n.adapt  = 5000          
)
...
summary_params_ar1 <- varnames(samples_ar1)
summary_params_ar1 <- summary_params_ar1[!grepl("^log_lik\\[", summary_params_ar1)]
samples_summary_ar1 <- samples_ar1[, summary_params_ar1]
summary_results_ar1 <- summary(samples_summary_ar1)

save(summary_results_ar1,
     file = file.path(out_dir, "summary_results_AR1.RData"))

final_table_ar1 <- cbind(
  Mean     = summary_results_ar1$statistics[, "Mean"],
  SD       = summary_results_ar1$statistics[, "SD"],
  Lower_95 = summary_results_ar1$quantiles[, "2.5%"],
  Upper_95 = summary_results_ar1$quantiles[, "97.5%"]
)

print(final_table_ar1)

write.csv(final_table_ar1,
          file = file.path(out_dir, "AR1_model_parameter_estimates.csv"),
          row.names = TRUE)

rmse_ar1 <- summary_results_ar1$statistics["rmse", "Mean"]
