...
model {
  ...
  gamma_alpha ~ dnorm(0.23446178, 200)
  gamma_beta ~ dnorm(0.00705636, 200)
  phi ~ dnorm(-0.03860632, 0.1) T(-1, 1)
...
inits_function <- function() {
  list(
    gamma_alpha = rnorm(1, 0.23446178, 0.05),
    gamma_beta = rnorm(1, 0.00705636, 0.02),
...
# 5. Save the 'samples' object to an RData file
save(samples,
     file = file.path(out_dir, "samples_new_model.RData"))
