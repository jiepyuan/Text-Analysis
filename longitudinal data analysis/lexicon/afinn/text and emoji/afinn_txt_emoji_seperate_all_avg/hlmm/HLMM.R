...
model {
  ...
  # intercept prior: 0.2898
  beta0 ~ dnorm(0.2898, 0.1) 
  
  # variance function parameters prior
  gamma0 ~ dnorm(log(pow(3.209, 2)), 0.1)
  gamma1 ~ dnorm(0.01036, 0.1)
...
jags_mod_hlmm <- jags.model(
  ...
  data     = jags_data_hlmm,
  inits    = list(inits_hlmm_1, inits_hlmm_2, inits_hlmm_3), 
  n.chains = 3,            
  n.adapt  = 5000          
)
...
summary_params_hlmm <- varnames(samples_hlmm)
summary_params_hlmm <- summary_params_hlmm[!grepl("^log_lik\\[", summary_params_hlmm)]
samples_summary_hlmm <- samples_hlmm[, summary_params_hlmm]
summary_results_hlmm <- summary(samples_summary_hlmm)

save(summary_results_hlmm,
     file = file.path(out_dir, "summary_results_HLMM.RData"))

final_table_hlmm <- cbind(
  Mean     = summary_results_hlmm$statistics[, "Mean"],
  SD       = summary_results_hlmm$statistics[, "SD"],
  Lower_95 = summary_results_hlmm$quantiles[, "2.5%"],
  Upper_95 = summary_results_hlmm$quantiles[, "97.5%"]
)

print(final_table_hlmm)

write.csv(final_table_hlmm,
          file = file.path(out_dir, "HLMM_model_parameter_estimates.csv"),
          row.names = TRUE)

rmse_hlmm <- summary_results_hlmm$statistics["rmse", "Mean"]
