...
sigma_int   <- 1.06948814   
sigma_slope <- 0.03905485   
rho         <- -0.211
...
model {
  ...
  gamma00 ~ dnorm(3.288760 , 0.1)
  gamma10 ~ dnorm(0.018287  , 0.1)
...
alpha_init_1 <- matrix(rep(c(3.288760, 0.018287), max(long_dat$id_idx)), 
                       ncol = 2, byrow = TRUE)

alpha_init_2 <- matrix(rep(c(3.00, 0.03), max(long_dat$id_idx)), 
                       ncol = 2, byrow = TRUE)
