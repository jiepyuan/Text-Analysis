...
# (0) load in data --------------------------------------------------------
dat <- read.csv("afinn_separate_pos_avg_txt_emoji.csv")
dat$id <- 1:nrow(dat) 
...
model {
  ...
  gamma_alpha ~ dnorm(3.288760, 200)
  gamma_beta ~ dnorm(0.018287   , 200)
  phi ~ dnorm(-0.023594 , 0.1) T(-1, 1)
...
inits_function <- function() {
  list(
    gamma_alpha = rnorm(1, 3.288760 , 0.05),
    gamma_beta = rnorm(1, 0.018287  , 0.02),
