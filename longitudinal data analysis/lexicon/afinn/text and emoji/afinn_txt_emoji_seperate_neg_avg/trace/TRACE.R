...
# (0) load in data --------------------------------------------------------
dat <- read.csv("afinn_separate_neg_avg_txt_emoji.csv")
dat$id <- 1:nrow(dat) 
...
model {
  ...
  gamma_alpha ~ dnorm(-3.718364, 200)
  gamma_beta ~ dnorm(0.019771  , 200)
  phi ~ dnorm(0.043698 , 0.1) T(-1, 1)
...
inits_function <- function() {
  list(
    gamma_alpha = rnorm(1, -3.718364, 0.05),
    gamma_beta = rnorm(1, 0.019771 , 0.02),
