...
# (0) load in data --------------------------------------------------------
dat <- read.csv("afinn_combine_txt_emoji.csv")
dat$id <- 1:nrow(dat) 
...
model {
  ...
  gamma_alpha ~ dnorm(-0.26726814, 200)
  gamma_beta ~ dnorm(0.04647982 , 200)
  phi ~ dnorm(0.1025914 , 0.1) T(-1, 1)
...
inits_function <- function() {
  list(
    gamma_alpha = rnorm(1, -0.26726814, 0.05),
    gamma_beta = rnorm(1, 0.04647982, 0.02),
