...
# (0) load in data --------------------------------------------------------
dat <- read.csv("afinn_separate_neg_avg_txt.csv")
dat$id <- 1:nrow(dat) 
...
model {
  ...
  gamma_alpha ~ dnorm(-3.749672, 200)
  gamma_beta ~ dnorm(0.016420 , 200)
  phi ~ dnorm(0.042288, 0.1) T(-1, 1)
...
inits_function <- function() {
  list(
    gamma_alpha = rnorm(1, -3.749672, 0.05),
    gamma_beta = rnorm(1, 0.016420, 0.02),
...
# Optional: Pareto k diagnostics
# print(pareto_k_table(model_loo))
