...
# (0) load in data --------------------------------------------------------
dat <- read.csv("afinn_separate_pos_avg_txt.csv")
dat$id <- 1:nrow(dat) 
...
model {
  ...
  gamma_alpha ~ dnorm(3.327737 , 200)
  gamma_beta ~ dnorm(0.023745  , 200)
  phi ~ dnorm(-0.023189, 0.1) T(-1, 1)
...
inits_function <- function() {
  list(
    gamma_alpha = rnorm(1, 3.327737 , 0.05),
    gamma_beta = rnorm(1, 0.023745 , 0.02),
