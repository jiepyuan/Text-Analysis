...
sigma_int   <- 0.91742902      
sigma_slope <- 0.06758531      
rho         <- -0.084
...
model {
  ...
  gamma00 ~ dnorm(3.327737 , 0.1)
  gamma10 ~ dnorm(0.023745 , 0.1)
...
alpha_init_1 <- matrix(rep(c(3.327737 , 0.016420), max(long_dat$id_idx)), 
                       ncol = 2, byrow = TRUE)

alpha_init_2 <- matrix(rep(c(3.307737 , 0.03), max(long_dat$id_idx)), 
                       ncol = 2, byrow = TRUE)
